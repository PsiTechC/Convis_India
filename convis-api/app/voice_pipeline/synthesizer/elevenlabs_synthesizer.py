...
class ElevenlabsSynthesizer(BaseSynthesizer):
    def __init__(self, voice, voice_id, model="eleven_turbo_v2_5", audio_format="mp3", sampling_rate="16000",
        ...
        self.ws_url = f"wss://{self.elevenlabs_host}/v1/text-to-speech/{self.voice}/multi-stream-input?model_id={self.model}&output_format={'ulaw_8000' if self.use_mulaw else 'mp3_44100_128'}&inactivity_timeout=1800&sync_alignment=true"
        ...
        self._interrupted = False  # Flag for interruption handling
    ...
    async def handle_interruption(self):
        """
        Handle user interruption - stop generating audio immediately
        NOTE: We only send flush, NOT close_context, to keep the WebSocket connection alive
        """
        try:
            logger.info("[ELEVENLABS] 🛑 Handling interruption - flushing buffer")
            
            # Mark that we're interrupted (temporarily)
            self._interrupted = True
            
            if self.websocket_holder.get("websocket"):
                try:
                    # Only send flush to clear buffered audio - DON'T close context
                    # close_context can cause WebSocket to become unresponsive
                    flush_message = {"text": "", "flush": True}
                    await self.websocket_holder["websocket"].send(json.dumps(flush_message))
                    logger.info("[ELEVENLABS] 🔇 Sent flush to ElevenLabs")
                except Exception as e:
                    logger.warning(f"[ELEVENLABS] Error sending flush: {e}")

            # Generate new context_id for next response
            self.context_id = str(uuid.uuid4())
            
            # Reset interrupted flag immediately - the voice pipeline controls interruption state
            self._interrupted = False
            
        except Exception as e:
            logger.warning(f"[ELEVENLABS] Error in handle_interruption: {e}")
            ...
            if text != "":
                for text_chunk in self.text_chunker(text):
                    ...
                    if self._interrupted:
                        logger.info("[ELEVENLABS] 🛑 Interrupted - stopping text synthesis")
                        return
                    if not self.should_synthesize_response(sequence_id):
                        logger.info(
        ...
        while True:
            try:
                ...
                if (self.websocket_holder["websocket"] is None or
                    ...
                    logger.info("WebSocket is not connected, waiting for reconnection...")
                    await asyncio.sleep(1)  # Reduced from 5s to 1s for faster recovery
                    continue

                response = await self.websocket_holder["websocket"].recv()
                data = json.loads(response)
                logger.debug(f"ElevenLabs response isFinal: {data.get('isFinal', False)}")
                ...
                if "isFinal" in data and data["isFinal"]:
                    ...
                    try:
                        ...
                        logger.debug(f'Last four char - {last_four_words_text} | current text - {current_normalize_text}')
                    ...
                    logger.debug("No audio data in the response")

            except websockets.exceptions.ConnectionClosed as e:
                # Don't break - wait for monitor_connection to reconnect
                logger.warning(f"[ELEVENLABS] WebSocket closed: {e}, waiting for reconnection...")
                await asyncio.sleep(1)
                continue
            except Exception as e:
