...
class DeepgramTranscriber(BaseTranscriber):
    def __init__(self, telephony_provider, input_queue=None, model='nova-2', stream=True, language="en", endpointing="400",
        ...
        self.api_key = kwargs.get("transcriber_key", os.getenv('DEEPGRAM_API_KEY', os.getenv('DEEPGRAM_AUTH_TOKEN')))
        ...
        logger.info(f"[DEEPGRAM] Initialized with noise_suppression_level={self.noise_suppression_level}, endpointing={endpointing}ms")
        if not self.stream:
        ...
        self.turn_counter = 0
        # Timeout tracking for stuck utterances - REDUCED for faster response
        self.last_interim_time = None
        self.interim_timeout = kwargs.get("interim_timeout", 2.0)  # Reduced to 2 seconds for faster finalization
        self.utterance_timeout_task = None

    def get_deepgram_ws_url(self):
        # OPTIMIZED for LOW LATENCY voice conversations
        logger.info(f"[DEEPGRAM] Using model: {self.model}, language: {self.language}")
        
        # Use a stable utterance_end_ms value - minimum 1000ms for reliability
        utterance_end = max(1000, int(self.endpointing) + 500)
        
        # SIMPLIFIED parameters for maximum compatibility
        # Remove vad_events and smart_format which may cause issues with certain models
        dg_params = {
            'model': self.model,
            ...
            'endpointing': str(max(500, int(self.endpointing))),  # Explicit endpointing
        }
        
        logger.info(f"[DEEPGRAM] Params: model={self.model}, utterance_end={utterance_end}ms, endpointing={self.endpointing}ms")
    ...
    async def deepgram_connect(self):
        ...
        try:
            ...
            if not self.api_key:
                logger.error("[DEEPGRAM] No API key found! Check DEEPGRAM_API_KEY environment variable")
            ...
            logger.info(f"[DEEPGRAM] Connecting with API key: {key_preview}...")
            ...
            # Log the full response if available for debugging
            logger.error(f"Invalid handshake during Deepgram websocket connection: {e}")
            if hasattr(e, 'response'):
                logger.error(f"[DEEPGRAM] Server response body: {e.response}")
            logger.error(f"[DEEPGRAM] API Key (first 8 chars): {self.api_key[:8] if self.api_key else 'NONE'}")
            logger.error(f"[DEEPGRAM] Check if API key has WebSocket/streaming access enabled")
